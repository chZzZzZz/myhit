...
const double INIT_T =5000;      //初始温度
const double RATE = 0.99;        //温度衰减率
const double FINAL_T = 0;    //终止温度
const int IN_LOOP = 2000;      //内层循环次数
const int OUT_LOOP =5000 ;       //外层循环次数
...
inline int totaldist(sequence q)        //计算总时间
 {  int pe=0;//惩罚时间pe
    ...
    for(int i=0 , k=0;i<20,k<20;i++,k++)
    {
     ...
     if(cost[i-1]<=q.s1&&cost[i]>q.s1)//t=29时刻的检修
     {
         cost[i]=q.s1+1+cost[i]-cost[i-1];
         int x=i;
         int a=q.s1+1-cost[i-1];
         for(x=i;x<N-1;x++)
         {
             cost[x+1]=a;
         }
     }
     if(cost[i-1]<=q.s2&&cost[i]>q.s2)//t=59时刻的检修
     {
         cost[i]=q.s2+1+cost[i]-cost[i-1];
         int y=i;
         int b=q.s2+1-cost[i-1];
         for(y=i;y<N-1;y++)
         {
             cost[y+1]+=b;
         }
...
int main()
 {
	...
	for(int i=0;i<N;i++)
	{
 	...
 	printf("\n");
    bestseq=sa();
    printf("最优维护时刻：%d,%d",bestseq.s1,bestseq.s2);
    printf("最优工件序列: %d\n", bestseq.time);
	for(int j=0;j<N;j++)
	{
